...
class connector(models.Model):
    ...
    @api.multi        
    def vehiculos(self):
        imp_obj = self.env['delsol.invoiced_vehicle_imp']
        imp_obj.import_vehicle()

    @api.multi        
    def planahorro(self):
        imp_obj = self.env['delsol.invoiced_planahorro']
        imp_obj.import_vehicle()
        
        
    def connect(self):
        server = ''
        if self.db_port == False:
            server = self.db_ip
        else:
            server = self.db_ip + ':' + self.db_port
        try:
            conn = pymssql.connect(#self.db_ip,self.db_user, self.password, self.db_name,as_dict=True)
                server, self.db_user, self.password, self.db_name,as_dict=True)
        except ValueError, e:
